...
def generate_pie_chart(df: pd.DataFrame, column, labels, title, legend_title, img_path):
    # Calculate the 8% threshold
    threshold = df[column].sum() * 0.08
...
def add_tablespace_details_table_to_doc(doc, df):
...
def add_schema_details_table_to_doc(doc, df):
...
def generate_reports(databases_info):
    ...
    for database_info in databases_info:
        ...
        generate_pie_chart(df, 'USED_SPACE_MB', df['TABLESPACE_NAME'], f'Used Space by Tablespace for {db_name}', 'Tablespaces', img_path)
        ...
        generate_pie_chart(schema_df, 'SCHEMA_SIZE_MB', schema_df['SCHEMA_NAME'], f'Schema Sizes for {db_name}', 'Schemas', img_path)
        doc.add_picture(img_path, width=Inches(6))
